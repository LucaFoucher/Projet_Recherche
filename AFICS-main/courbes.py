...
plt.xlabel('Angle')
plt.ylabel('Count')
plt.title('Courbe du nombre d apparition de chaque angle')


fig, ax1 = plt.subplots()
ax2 = ax1.twinx()
ax1.plot(rdf['Distance (A)'], rdf['Count'], 'g-', label='Courbe 1')
ax2.plot(rdf['Distance (A)'], rdfint['Integral'], 'b-', label='Courbe 2')
ax2.set_ylim(0, 12)


plt.figure(3)
for i in range(rmsd.shape[1]):
    plt.plot(x,rmsd[rmsd.columns[i]])
